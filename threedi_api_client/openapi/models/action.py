...
class Action(object):
    ...
    def simulation_runner(self):
        ...
        Only allowed for name values: 'start', 'initialize' or 'queue Simulation runner version used to execute the simulation. If not filled in the default version is used. (Note: used for testing new calccore releases on the staging environment. On production only the default version can be picked)  * `3.5.6-3.7.2` - 3.5.6-3.7.2 * `3.2.17-2.3.6` - 3.2.17-2.3.6  # noqa: E501
