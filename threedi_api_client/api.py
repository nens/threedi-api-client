...
try:
    ...
    from threedi_api_client.aio.aiohttp_retry import RetryClient
except ImportError:
    AsyncApiClient = None
...
class ThreediApi:
    ...
    Args:
        ...
        rest_config (dict) an optional dictionary containing configuration options for
          the REST client, examples:
         - "retries" for synchronous operation: int, tuple, or an urllib3.util.Retry instance
           for example: ``import urllib3; urllib3.util.Retry(3, backoff_factor=1.0)``
         - "retries" for asynchronous operation: aiohttp_retry.RetryOptionsBase instance
           for example: ``from threedi_api_client.aio.aiohttp_retry import ExponentialRetry; ExponentialRetry(3)``
           Note that aiohttp_retry has been included in threedi-api-client.
    ...
    def __init__(self, env_file=None, config=None, version="v3", asynchronous=False, rest_config=None):
