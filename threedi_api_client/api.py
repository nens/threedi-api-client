...
class ThreediApi:
    ...
    Notes:
        **Timeouts**
        ...
        paramater on every api request. It is currently not possible to configure a default timeout.

        **Retry policy**

        It is common to configure a retry policy to prevent exceptions due to the service being
        temporarily unavailable. The ``ThreediApi`` supports this through the ``rest_config``
        ...
        >>> api = ThreediApi(..., rest_config={"retries": 3})

        For synchronous usage, supply a ``urllib3.util.Retry`` object
        ...
        >>> api = ThreediApi(..., rest_config={"retries": policy})

        For asynchronous usage, supply a ``aiohttp_retry.util.Retry`` object. See
        ...
        if asynchronous:
            ...
            if configuration.retries is not None:
                ...
                if isinstance(configuration.retries, int):
                    configuration.retries = ExponentialRetry(attempts=configuration.retries)
